---
category: insights
commodities:
- iron_ore
- coal
- grain
- crude_oil
- products
- gas
date: '2025-05-19'
doc_id: breakwave_insights_insights_2025-05-19_2025_05_19_chinas_coal_imports_from_the_united_states_could_now_increas
document_type: insights_note
key_entities:
- China
- Australia
keywords:
- china
- australia
- coal
- imports
- tons
- million
- down
- february
- tons.
- year-on-year
- totaled
- states
linked_assets_discovered: 1
linked_assets_failed: 0
linked_assets_ingested: 0
linked_assets_mirrored: 0
linked_assets_skipped: 1
market_tone: neutral
numeric_observation_count: 1
numeric_observations:
- section: Main
  section_type: null
  source_line: March (data is still pending for April) saw coal imports from Indonesia
    total 18 million tons. This is up from February by 2.7 million tons (18%) but
    is down year-on-year by 1.8 million tons (-9%). Imports from Russia, China's second
    largest source of coal imp
  unit: pct
  values:
  - 18.0
  - 2.7
  - 18.0
  - 1.8
  - -9.0
  - 7.3
  - 1.0
  - 16.0
  - 400000.0
  - 6.0
regions:
- china
- brazil
- australia
- atlantic
- pacific
- meg
- west_africa
- europe
section_count: 1
signals: {}
source: breakwave_insights
source_path: reports/breakwave/2025/2025-05-19_chinas-coal-imports-from-the-united-states-could-now-increas.html
source_stem: 2025-05-19_chinas-coal-imports-from-the-united-states-could-now-increas
source_url: https://www.breakwaveadvisors.com/insights/2025/5/19/chinas-coal-imports-from-the-united-states-could-now-increase
summary: 'Main

  By Jeffrey Landsberg

  As we discussed in Commodore Research''s most recent Weekly China Report, Indonesia
  not surprisingly has remained China''s primary source of coal imports. Also noteworthy
  is that imports from Australia have continued to plummet in recent months, and imports
  from the United States have almost fallen to zero. Going forward, though, it is
  now certainly possible that imports from the United States could drastically increase
  from trade deal agreements.'
themes:
- china
- australia
- coal
- imports
- tons
- million
title: Chinas Coal Imports From The United States Could Now Increase
vessel_classes:
- capesize
- panamax
- supramax
- handysize
- vlcc
- suezmax
- aframax
---

## Summary
Main
By Jeffrey Landsberg
As we discussed in Commodore Research's most recent Weekly China Report, Indonesia not surprisingly has remained China's primary source of coal imports. Also noteworthy is that imports from Australia have continued to plummet in recent months, and imports from the United States have almost fallen to zero. Going forward, though, it is now certainly possible that imports from the United States could drastically increase from trade deal agreements.

## Main
By Jeffrey Landsberg
As we discussed in Commodore Research's most recent Weekly China Report, Indonesia not surprisingly has remained China's primary source of coal imports. Also noteworthy is that imports from Australia have continued to plummet in recent months, and imports from the United States have almost fallen to zero. Going forward, though, it is now certainly possible that imports from the United States could drastically increase from trade deal agreements.
March (data is still pending for April) saw coal imports from Indonesia total 18 million tons. This is up from February by 2.7 million tons (18%) but is down year-on-year by 1.8 million tons (-9%). Imports from Russia, China's second largest source of coal imports during the last several months, totaled 7.3 million tons. This is up from February by 1 million tons (16%) and is up year-on-year by 400,000 tons (6%). Imports from Australia totaled 4.4 million tons. This is down from February by 400,000 tons (-8%) and is down year-on-year by 700,000 tons (-14%). Imports from Australia peaked in November and have fallen by over 50% since then. Imports from Mongolia totaled 6.8 million tons. This is up from February by 1.7 million tons (33%) and is up year-on-year by 300,000 tons (5%). Imports from the United States totaled 200,000 tons. This is down from February by 1.1 million tons (-85%) and is down year-on-year by 400,000 tons (-67%).