...
| [`data/futures/sgx_*_futures_history.csv`](file:///c:/Users/Dell/Github/Shipping/data/futures/sgx_cape_futures_history.csv) | **Full Contract Lives** (4 files) | 03-2017 | ~324,547 | Complete SGX FFA settlement history per contract (`contract, expiry_month, expiry_year, date, price, volume, expiry_date`), backfilled & refreshed Mon–Thu via `scripts/expansion_sgx_history_backfill.py`. Real settlements exist for contracts expiring Jan 2024 onward (SGX serves zero-filled rows for older lives; the collector auto-skips them). Surfaced in the frontend via the **Contract Archive** selector on the SGX FFA Forward Curve — lazy-loaded per vessel class (~2–6 MB), then session-cached. Expired-contract drilldowns merge archive + live snapshot with live-file precedence on duplicate dates. |
