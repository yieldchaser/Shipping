...
def build_candidate_urls():
    """Build list of expected press release URLs dynamically up to current month."""
    candidates = []
    now = datetime.now()
    for y in range(2024, now.year + 1):
        max_m = now.month if y == now.year else 12
        for idx in range(max_m):
            m_code, m_name = MONTHS_MAP[idx]
            url = f"https://worldsteel.org/media/press-releases/{y}/{m_name}-{y}-crude-steel-production/"
            candidates.append((f"{y}-{m_code}-01", y, m_code, url))
    return candidates
...
def run_pipeline():
    ...
    latest_obs = {
        ...
        "world_total_mt": float(last_row["world_total_mt"]) if pd.notnull(last_row["world_total_mt"]) else None,
        "china_mt": float(last_row["china_mt"]) if pd.notnull(last_row["china_mt"]) else None,
        "india_mt": float(last_row["india_mt"]) if pd.notnull(last_row["india_mt"]) else None,
        "united_states_mt": float(last_row["united_states_mt"]) if pd.notnull(last_row["united_states_mt"]) else None,
        "japan_mt": float(last_row["japan_mt"]) if pd.notnull(last_row["japan_mt"]) else None,
    }
    ...
    meta = {
        ...
        "date_span": [str(df["date"].min()), str(df["date"].max())],
        "latest_data_through": str(df["date"].max()),
...
def update_manifest(df):
    ...
    entry_data = {
        ...
        "fetch_script": "scripts/scrapers/fetch_world_steel_production.py",
