...
dates (YYYY-MM-DD) and ISO weeks (1-53), eliminating week-00 dropped weeks.
Outputs:
...
def parse_vessel_activity():
    ...
    for idx in range(2, len(df)):
        ...
        iso_year, iso_week, _ = raw_dt.isocalendar()
        year = iso_year
        month = raw_dt.month

        # Use ISO calendar week directly (1-53), avoiding %W week-00 dropped weeks
        try:
            week_val = df.iloc[idx]["Unnamed: 18"]
            week = int(week_val) if (pd.notna(week_val) and int(week_val) > 0) else iso_week
        except Exception:
            week = iso_week
    ...
    entries = [
        {
            ...
            "fetch_script": "scripts/scrapers/fetch_usda_grain_queues.py",
