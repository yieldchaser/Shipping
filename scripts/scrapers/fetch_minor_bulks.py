...
====================================================================
Synthesizes monthly trade flows across the 7 major minor bulk commodities
using authoritative primary sources and Comtrade fallback:
1. Sugar: Brazil exports (MDIC ComexStat NCM 1701) — Supramax/Handy driver
2. Urea / Fertiliser: India imports (DGCI&S TradeStat HS 3102)
3. Fertiliser (NPK/DAP): Brazil imports (MDIC ComexStat HS 3105 CIF)
4. Alumina: China imports (GACC / chinadata.live HS 28182000 + SMM)
5. Nickel Ore: Philippines exports (PSA OpenSTAT HS 2604)
6. Scrap Steel: Türkiye imports (TurkStat General Trade HS 7204 + SteelOrbis fallback)
7. Cement & Clinker: Türkiye exports (TurkStat General Trade HS 2523)
...
def run_minor_bulks_harvest():
    ...
    # Dynamically generate monthly periods up to current month
    now = datetime.now()
    periods = []
    for y in range(2022, now.year + 1):
        max_m = now.month if y == now.year else 12
        for m in range(1, max_m + 1):
            periods.append(f"{y}{m:02d}")

    logging.info("Starting minor bulks harvest across %d flows and %d periods...",
    ...
    for flow in TARGET_FLOWS:
        ...
        # Skip Cement and Scrap from Comtrade: routed to TurkStat General Trade per §2.10
        if c_name in ("Cement / Clinker", "Scrap Steel"):
            logging.info("Routing %s to TurkStat general trade scraper (§2.10); skipping Comtrade.", c_name)
            continue

        # Skip Alumina from Comtrade: routed to chinadata.live + SMM per §2.6
        if c_name == "Alumina":
            logging.info("Routing Alumina to GACC / chinadata.live pipeline (§2.6); skipping Comtrade.")
            continue
        ...
        for p in periods:
            ...
            # Check India TradeStat for Urea
            if c_name == "Urea / Fertiliser":
                try:
                    from scripts.scrapers.fetch_india_tradestat import fetch_month_row
                    row_ts = fetch_month_row(int(p[:4]), int(p[4:6]), hs="3102")
                    if row_ts and row_ts.get("metric_tonnes", 0) > 0:
                        records.append(row_ts)
                        total_fetched += 1
                        continue
                except Exception as exc:
                    logging.debug("TradeStat check skipped for %s: %s", p, exc)

            # Fall back to UN Comtrade preview
    ...
    if df.empty:
        logging.error("No records found for minor bulks!")
        return
    ...
    update_manifest(len(df), str(df["date"].min()), str(df["date"].max()))
...
def update_manifest(row_count, min_date, max_date):
    ...
    entry_data = {
        ...
        "status": "LIVE",
        "source_name": "TurkStat, DGCI&S India, PSA Philippines, GACC China, Brazil MDIC ComexStat",
        "source_url": "https://bi.tuik.gov.tr",
        "fetch_method": "Multi-Source National Statistics Harvesters",
        "fetch_script": "scripts/scrapers/fetch_minor_bulks.py",
