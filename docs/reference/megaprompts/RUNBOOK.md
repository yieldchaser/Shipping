...
| **2nd — RUN THIS** | **`17-finish-line.md`** + `QUEUE-17.md` + appendices `17-APPENDIX-A..D` | Continuous, queue-driven run. Phase 0 writes the proof tests first. Restores ~30 modules blanked since Round 1 (loaders reading non-existent columns, incl. SGX), wires every updater, removes internal notes from the UI, fixes typed/wrong numbers, adds Baltic codes, rebuilds Tracking, then speed, tooltips and data completeness. Uses 15 and 16 as sub-specs. |
| (sub-spec) | **`15-data-complete-and-self-updating.md`** | Wires all 11 unscheduled fetchers into GitHub workflows (and stops `usda_weekly.yml` overwriting the new queue file), pulls August 2026, adds China tonnes (GACC bulletin), JODI tanker flows, EIA LNG, India coal (TradeStat), ABS Australia, and a generated coverage matrix. **Absorbs Prompt 10.** |
| (sub-spec) | **`16-fast-clean-explained.md`** | Tab speed (ETFs 2 s freeze on every revisit; Tracking loads 18.7 MB), tooltips ≥95% per tab, removal of generic-AI design patterns. **Absorbs Prompt 12.** |
