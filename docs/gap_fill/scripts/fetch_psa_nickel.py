#!/usr/bin/env python3
"""
Philippines nickel ore exports (HS 2604) from PSA OpenSTAT PXWeb API – no key needed.
...
def post(table, body):
    req = urllib.request.Request(B + table, data=json.dumps(body).encode(), headers={"Content-Type": "application/json", "User-Agent": "curl/8.5.0"})
    return urllib.request.urlopen(req, timeout=240).read().decode("latin1")


def codes(table, prefix):
    body = {"query": [{"code": "Commodity Code", "selection": {"filter": "all", "values": ["*"]}},
...
def monthly(table, prefix="2604"):
    body = {"query": [{"code": "Commodity Code", "selection": {"filter": "item", "values": codes(table, prefix)}}],
            "response": {"format": "csv"}}
    ...
    for y in years:
        ...
        for col in q.index:
            ...
            if pd.isna(q[col]) or q[col] == 0:
                continue
            rows.append(dict(date=f"{y}-{m:02d}-01", period=f"{y}{m:02d}", commodity="Nickel Ore", trade_flow="Exports",
                             reporter_country="Philippines", partner_country="World", hs_code=2604,
                             metric_tonnes=round(q[col] / 1000, 2), value_usd=float(v.get(col)),
