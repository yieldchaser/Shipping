...
| [`fearnleys_tanker_routes_daily.json`](file:///c:/Users/Dell/Github/Shipping/data/derived/fearnleys_tanker_routes_daily.json) | Broker Desk S3 daily tanker routes: 126 series (VLCC 26 / Suezmax 37 / Aframax 43 / Dirty 9 / 1Y T/C 3 / Fuel Oil 5 / WEEKLY VLCC 2 / MEG Fixture Count), chronological `[epoch_ms,value]` pts at UTC midnights, per-klass index; monthly-twin codes reused so the UI can cross-reference; ws + tce/usd unit branches kept as separate series (source's 2023-05 taxonomy switch ended the tce branch 2023-04-25); 4 stale subtypes excluded and recorded in meta.excluded | 2018-05 → live | Daily (`data_expansion.yml`) | `scripts/fearnleys/build_tanker_routes_daily.py` from `fearnpulse_rates_full.csv` | Active (3.6 MB; 191,421 pts; `--verify` floors) |
