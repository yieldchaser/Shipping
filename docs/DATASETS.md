...
| [`fearnleys_tanker_routes_daily.json`](file:///c:/Users/Dell/Github/Shipping/data/derived/fearnleys_tanker_routes_daily.json) | Broker Desk S3 daily tanker routes: 126 series (VLCC 26 / Suezmax 37 / Aframax 43 / Dirty 9 / 1Y T/C 3 / Fuel Oil 5 / WEEKLY VLCC 2 / MEG Fixture Count), chronological `[epoch_ms,value]` pts at UTC midnights, per-klass index; monthly-twin codes reused so the UI can cross-reference; ws + tce/usd unit branches kept as separate series (source's 2023-05 taxonomy switch ended the tce branch 2023-04-25); 4 stale subtypes excluded and recorded in meta.excluded; the 8 frozen-real tce branches (MEG/FEAST, MEG/USG, WAFR/FEAST VLCC + BLSEA/MED, CEYHAN/USG, CROSS MED, WAFR/UKC, WAFR/USG Suezmax) were tested for WS-to-TCE continuation from their live ws twins (OLS on the shared window, R2 gate 0.90) — all 8 measured below gate (best VLCC MEG/FEAST 0.899; the 5 Suezmax tce branches carry a 721–1007-day identical-value plateau from mid-2020, i.e. the source itself stopped assessing them) so none emitted; each pair's measured fit is recorded in `meta.skipped_with_reason` | 2018-05 → live | Daily (`data_expansion.yml`) | `scripts/fearnleys/build_tanker_routes_daily.py` from `fearnpulse_rates_full.csv` | Active (3.6 MB; 191,426 pts; `--verify` floors) |
...
| [`data/congestion/portwatch_port_congestion.csv`](file:///c:/Users/Dell/Github/Shipping/data/congestion/portwatch_port_congestion.csv) | Core12 measured port activity (Qingdao, Ningbo, Port Hedland, Newcastle, Singapore, Rotterdam, Houston, Tubarao, Santos, Rizhao, Hay Point, Qinhuangdao + All): daily port calls by class + dry-bulk/tanker import/export tonnages (kt), real IMF PortWatch AIS observations only — no waiting-time/anchored-ship synthesis (withdrawn 2026-08-25). NOTE 2026-09-09: `port_calls_daily_v2.csv` was a byte-identical duplicate of this file and has been retired; producers updated to the canonical name. | 2019–live | Daily | IMF PortWatch ArcGIS `Daily_Ports_Data` FeatureServer (`fetch_portwatch_port_activity.py`) | Active |
| [`data/geospatial/portwatch_ports_master.csv`](file:///c:/Users/Dell/Github/Shipping/data/geospatial/portwatch_ports_master.csv) | FULL 2065-port universe infrastructure facts, verbatim: portid/name/country/ISO3/continent, lat/lon, vessel counts by 6 classes, industry_top1..3, country maritime import/export shares, LOCODE, pageid | 2026 snapshot | Manual refresh | IMF PortWatch ArcGIS `PortWatch_ports_database` (`fetch_portwatch_ports_expanded.py --refresh`) | Active |
| [`data/congestion/chokepoints_master.csv`](file:///c:/Users/Dell/Github/Shipping/data/congestion/chokepoints_master.csv) | All 28 PortWatch chokepoints, same infrastructure schema as the ports universe (Suez, Panama, Hormuz, Malacca, Bab el-Mandeb, Bosphorus, ...) | 2026 snapshot | Manual refresh | IMF PortWatch ArcGIS `PortWatch_chokepoints_database` (`fetch_portwatch_ports_expanded.py --refresh`) | Active |
| [`data/congestion/portwatch_disruptions.csv`](file:///c:/Users/Dell/Github/Shipping/data/congestion/portwatch_disruptions.csv) | 132+ disruption events (TC/EQ/WF/OT): epoch-ms → ISO fromdate/todate, `ongoing` flag (todate null = active event, e.g. Red Sea tensions RED since 2023-12-16), alertlevel/severitytext, affectedports kept raw (semicolon ids), affectedpopulation | 2019–live | Daily refresh | IMF PortWatch ArcGIS `portwatch_disruptions_database` (`fetch_portwatch_ports_expanded.py --refresh`) | Active |
| [`data/congestion/port_calls_daily_expanded.csv`](file:///c:/Users/Dell/Github/Shipping/data/congestion/port_calls_daily_expanded.csv) (+ `.parquet`; prior years sharded as `port_calls_daily_expanded_<YYYY>.csv.gz`) | EXPANDED daily port calls for the full 2065-port universe in the six-class source schema (portcalls_{container,dry_bulk,general_cargo,roro,tanker,cargo,total} + import/export by all six classes, incl. offshore terminals): current year is the hot CSV (+ zstd parquet), prior calendar years are gzip CSV shards — full history committed, sharded only to respect GitHub's 100 MB/file hard limit (~5.8M rows ≈ 726 MB raw). ObjectId-keyset pagination (deep offsets are rejected by the layer), rate-limited pages, per-year source reconciliation recorded in `portwatch_expanded_manifest.json` | 2019–live | Daily refresh (hot file) | IMF PortWatch ArcGIS `Daily_Ports_Data` (`fetch_portwatch_ports_expanded.py`) | Active |
| [`data/geospatial/voyage_history_fixturegrounded.csv`](file:///c:/Users/Dell/Github/Shipping/data/geospatial/voyage_history_fixturegrounded.csv) (+ `.parquet`) | Fixture-grounded voyage history — one row per REPORTED Fearnleys leg: vessel, imo AS REPORTED (blank stays blank — NO generated IMOs), vessel_class from segment/department, commodity, load/discharge port + resolved portid/LOCODE/match-quality (legacy alias map + 2065-port universe resolver), leg date, per-vessel leg sequence, great-circle distance_nm only when BOTH ends resolve. NO lat/lon polylines, NO status/days_waiting, NO DWT constants. Label downstream: "reported fixture voyages — not AIS positions" | 1974–live | Rebuild (idempotent) | `fearnleys_fixtures_full.csv` via `build_voyage_history.py` | Active |
